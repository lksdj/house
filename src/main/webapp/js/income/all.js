...
function getAll(x,y){
	...
	table.render({
		elem:'#test',//要绑定数据的地方
		url:path+"income.do?method=all&begin="+x+"&pages="+y,
		cellMinWidth:50,//最小列宽
		cols:[[
			{field:'nid',title:'编号',sort:true,align:'center'},
			{field:'nmoney',title:'收入金额',align:'center'},
			{field:'nsource',title:'来源条目',align:'center'},
			{field:'ntime',title:'时间 ',align:'center'},
			{field:'nremark',title:'备注时间',align:'center'},
...
function getUpp(nid){
		...
		layer.open({
			...
			maxmin:true,
			content:[path+"income/upd.jsp?nid="+nid],
			end:function(){
				getAll(begin, pages);
			}
		});
