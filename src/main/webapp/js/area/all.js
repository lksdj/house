...
function getAll(x,y){
	...
	table.render({
		elem:'#test',//要绑定数据的地方
		url:path+"area.do?method=all&begin="+x+"&pages="+y,
		cellMinWidth:50,//最小列宽
		cols:[[
			{field:'aid',title:'片区编号',sort:true,align:'center'},
			{field:'aname',title:'片区名称',align:'center'},
...
function getUpp(aid){
		...
		layer.open({
			...
			maxmin:true,
			content:[path+"area/upd.jsp?aid="+aid],
			end:function(){
				getAll(begin, pages);
			}
		});
...
function getDel(){
	$("body").on("click","#tab #del",function(){
		//得到修改对象主键值
		var jid=$(this).parent().parent().find("td:eq(1)").text();
		//弹出一个询问层
