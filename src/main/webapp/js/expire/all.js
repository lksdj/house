...
function getAll(x,y,sid,aid,htype,ctel){
	...
	table.render({
		elem:'#test',//要绑定数据的地方
		url:path+"charge.do?method=selectExpire&begin="+x+"&pages="+y+"&sid="+sid+"&aid="+aid+"&htype="+htype+"&ctel="+ctel,
...
function getDetail(rid){
	layer.open({
		...
		maxmin:true,
		content:[path+"charge/detail.jsp?rid="+rid],
		end:function(){
			getAll(begin,pages,sid,aid,htype,ctel);
		}
	});
...
function getRecord(rid){
		...
		layer.open({
			...
			skin:'layui-layer-white',
			maxmin:true,
			content:[path+"charge/record.jsp?rid="+rid],
			end:function(){
				getAll(begin,pages,sid,aid,htype,ctel);
			}
		});
...
function getRelet(rid){
	...
	layer.open({
		...
		skin:'layui-layer-white',
		maxmin:true,
		content:[path+"expire/relet.jsp?rid="+rid],
		end:function(){
			getAll(begin,pages,sid,aid,htype,ctel);
		}
	});
